def get_values(*names):
            import json
            _all_values = json.loads("""{"num_samples":24,"starting_vol":50,"elution_vol":50,"park_tips":true,"mag_gen":"magdeck","m300_mount":"left","protocol_filename":"sci-omegabiotek-magbind"}""")
...
metadata = {
    "protocolName": "Mag-BindÂ® Viral DNA/RNA 96 Kit",
...
def run(ctx):
    """Protocol."""
    [num_samples, starting_vol, elution_vol, park_tips, mag_gen, m300_mount] = get_values(
        "num_samples", "starting_vol", "elution_vol", "park_tips", "mag_gen", "m300_mount"
    )
    if num_samples % 8 != 0:
    ...
    magdeck.disengage()
    magplate = magdeck.load_labware("nest_96_wellplate_2ml_deep", "deepwell plate")
    tempdeck = ctx.load_module("Temperature Module Gen2", "1")
    elutionplate = tempdeck.load_labware("opentrons_96_aluminumblock_nest_wellplate_100ul", "elution plate")
    waste = ctx.load_labware("nest_1_reservoir_195ml", "6", "Liquid Waste").wells()[0].top()
    res1 = ctx.load_labware("nest_12_reservoir_15ml", "2", "reagent reservoir 1")
    ...
    if park_tips:
        parkingrack = ctx.load_labware("opentrons_96_tiprack_300ul", "5", "tiprack for parking")
        parking_spots = parkingrack.rows()[0][:num_cols]
    else:
        tips300.insert(0, ctx.load_labware("opentrons_96_tiprack_300ul", "5", "200Âµl filtertiprack"))
    ...
    def elute(vol, park=True):
        ...
        magdeck.engage(height=MAG_HEIGHT)
        ctx.delay(minutes=2, msg="Incubating on MagDeck for " + str(settling_time) + " minutes.")
